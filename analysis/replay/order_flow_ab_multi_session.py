"""Comparador passivo de múltiplas sessões A/B de Order Flow."""
...
class OrderFlowABMultiSessionReport:
    sessions: int = 0
    samples: int = 0
...
class OrderFlowABMultiSessionComparator:
    VERSION = "RC1-ORDER-FLOW-AB-MULTI-SESSION"
    MIN_SESSIONS = 3
    MIN_SAMPLES = 300
    MAX_STABLE_SPREAD = 1.0

    def compare(self, reports) -> OrderFlowABMultiSessionReport:
        ...
        return OrderFlowABMultiSessionReport(
            sessions=sessions,
            samples=samples,
    ...
    def _stability(self, *, sessions, samples, effects, spread, validity_sessions):
        ...
        if validity_sessions or spread > self.MAX_STABLE_SPREAD:
            return "INCONSISTENT"
        meaningful = [e for e in effects if e != "NO_DATA"]
        if not meaningful:
            return "INSUFFICIENT_DATA"
    ...
    def _validate(reports):
        required = ("samples", "average_delta", "dominant_effect", "grade_changes", "validity_changes")
        for report in reports:
            if not all(hasattr(report, field) for field in required):
                raise TypeError("Relatório de sessão A/B de Order Flow inválido.")
