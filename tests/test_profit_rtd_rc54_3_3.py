...
def run():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        assert r['drop_runs'][0]['sample_count'] == 2
        ...
        precedence = dict(payload)
        precedence['samples'] = list(payload['samples'])
        precedence['samples'][0] = dict(precedence['samples'][0], trade_context_ready=False)
        path.write_text(json.dumps(precedence), encoding='utf-8')
        r = audit(path)
        assert r['ready_samples'] == 1
        assert r['drop_samples'] == 4
        ...
        gaps = dict(payload)
        gaps['samples'] = [
            _sample(1, False, trend='SIDEWAYS', bias='NONE', valid=True),
            _sample(10, False, trend='SIDEWAYS', bias='NONE', valid=True),
        ]
        path.write_text(json.dumps(gaps), encoding='utf-8')
        r = audit(path)
        assert r['longest_drop_run'] == 2
        assert r['drop_runs'] == [{'start_cycle': 1, 'end_cycle': 10, 'sample_count': 2}]
