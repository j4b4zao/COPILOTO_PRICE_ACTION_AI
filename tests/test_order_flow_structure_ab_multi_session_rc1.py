from types import SimpleNamespace

import pytest

from analysis.replay.order_flow_structure_ab_multi_session import (
    OrderFlowStructureABMultiSessionComparator,
)


def _report(
    samples=100,
    delta=0.5,
    confidence=0.8,
    effect="POSITIVE",
    grades=0,
    validity=0,
):
    return SimpleNamespace(
        samples=samples,
        average_delta=delta,
        average_confidence=confidence,
        dominant_effect=effect,
        grade_changes=grades,
        validity_changes=validity,
    )


def test_requires_three_sessions():
    result = OrderFlowStructureABMultiSessionComparator().compare([_report(), _report()])
    assert result.stability == "INSUFFICIENT_DATA"
    assert result.recommendation == "COLLECT_MORE_DATA"


def test_requires_300_samples():
    reports = [_report(samples=90) for _ in range(3)]
    result = OrderFlowStructureABMultiSessionComparator().compare(reports)
    assert result.samples == 270
    assert result.recommendation == "COLLECT_MORE_DATA"


def test_stable_positive_sessions():
    reports = [_report(delta=0.4), _report(delta=0.5), _report(delta=0.6)]
    result = OrderFlowStructureABMultiSessionComparator().compare(reports)
    assert result.stability == "STABLE_POSITIVE"
    assert result.recommendation == "KEEP_OBSERVING"


def test_stable_negative_sessions():
    reports = [
        _report(delta=-0.4, effect="NEGATIVE"),
        _report(delta=-0.5, effect="NEGATIVE"),
        _report(delta=-0.6, effect="NEGATIVE"),
    ]
    result = OrderFlowStructureABMultiSessionComparator().compare(reports)
    assert result.stability == "STABLE_NEGATIVE"


def test_stable_neutral_sessions():
    reports = [_report(delta=0.0, effect="NEUTRAL") for _ in range(3)]
    result = OrderFlowStructureABMultiSessionComparator().compare(reports)
    assert result.stability == "STABLE_NEUTRAL"


def test_conflicting_effects_are_inconsistent():
    reports = [
        _report(effect="POSITIVE"),
        _report(delta=-0.3, effect="NEGATIVE"),
        _report(delta=0.0, effect="NEUTRAL"),
    ]
    result = OrderFlowStructureABMultiSessionComparator().compare(reports)
    assert result.stability == "INCONSISTENT"
    assert result.recommendation == "REVIEW_BEFORE_ENABLE"


def test_excessive_spread_is_inconsistent():
    reports = [_report(delta=0.1), _report(delta=0.4), _report(delta=0.9)]
    result = OrderFlowStructureABMultiSessionComparator().compare(reports)
    assert result.delta_spread == 0.8
    assert result.stability == "INCONSISTENT"


def test_validity_change_forces_review():
    reports = [_report(), _report(validity=1), _report()]
    result = OrderFlowStructureABMultiSessionComparator().compare(reports)
    assert result.sessions_with_validity_changes == 1
    assert result.recommendation == "REVIEW_BEFORE_ENABLE"


def test_weighted_delta_and_confidence_use_sample_counts():
    reports = [
        _report(samples=100, delta=0.6, confidence=0.9),
        _report(samples=200, delta=0.3, confidence=0.6),
        _report(samples=100, delta=0.0, confidence=0.3),
    ]
    result = OrderFlowStructureABMultiSessionComparator().compare(reports)
    assert result.weighted_average_delta == 0.3
    assert result.weighted_average_confidence == 0.6
    assert result.samples == 400


def test_invalid_contract_is_rejected_and_input_is_not_mutated():
    reports = [_report(), _report(), _report()]
    before = list(reports)
    OrderFlowStructureABMultiSessionComparator().compare(reports)
    assert reports == before
    with pytest.raises(TypeError):
        OrderFlowStructureABMultiSessionComparator().compare([object()])
