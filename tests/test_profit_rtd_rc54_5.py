...
def run():
    with tempfile.TemporaryDirectory() as td:
        ...
        samples = [_sample(100 - i * 5) for i in range(6)]
        for sample in samples:
            sample['trade_context_ready'] = False
        precedence.write_text(json.dumps(_payload(samples)), encoding='utf-8')
        rp = accumulate([precedence], min_occurrences=1, min_sessions=1)
        assert rp['buckets'] == {}

        invalid = Path(td) / 'invalid.json'
        invalid.write_text(json.dumps(dict(_payload([_sample(100)]), data_ready=False)), encoding='utf-8')
        try:
            accumulate([invalid], min_occurrences=1, min_sessions=1)
        except ValueError as exc:
            assert 'RC54_5_REQUIRES_DATA_READY_SESSION' in str(exc)
        else:
            raise AssertionError('RC54.5 must reject technical data failure')
