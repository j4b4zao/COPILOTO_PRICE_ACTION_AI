...
def make_sample(price):
    ...
    price_reason = 'OK' if price is not None else 'PRICE_MISSING'
    return _sample(snapshot, source, quality, price, 1, 1, price_reason)


def test_price_is_persisted_without_fabrication():
    row = make_sample(176845.0)
    assert row['last_price'] == 176845.0
    assert row['price_read_attempts'] == 1
    assert row['price_read_reason'] == 'OK'
...
def test_missing_price_remains_missing():
    ...
    assert row['price_read_attempts'] == 1
    assert row['price_read_reason'] == 'PRICE_MISSING'
