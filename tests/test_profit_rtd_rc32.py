...
def ctx(*, alignment="NEUTRAL", dominance=0.0, imbalance=0.0, status="READY"):
...
def test_shadow_detects_bullish_without_changing_official():
    result = OrderFlowShadowCalibration(delta_threshold=0.35, book_threshold=0.062149).evaluate(
        ctx(alignment="NEUTRAL", dominance=0.60, imbalance=0.08)
    )
...
def test_shadow_detects_bearish_and_divergent():
    shadow = OrderFlowShadowCalibration(delta_threshold=0.35, book_threshold=0.062149)
    bearish = shadow.evaluate(ctx(dominance=-0.50, imbalance=-0.07))
    divergent = shadow.evaluate(ctx(dominance=0.50, imbalance=-0.07))
...
def test_shadow_keeps_neutral_when_source_not_ready():
    result = OrderFlowShadowCalibration().evaluate(
        ctx(alignment="NEUTRAL", dominance=0.80, imbalance=0.20, status="DEGRADED")
    )
