...
def test_group_by_state():
    ...
    assert recorder.scenario_summary()["by_state"]["CONFLICT"]["samples"] == 1


def test_scenario_combined_filters():
    recorder = MicrostructureConfluenceReplayRecorder()
    recorder.add_sample(_sample(independent=3, quality="HIGH", correlated=0))
...
def test_csv_export(tmp_path):
    ...
    path = recorder.export_csv(tmp_path / "microstructure.csv")
    assert path.exists()
    assert "independent_evidence_count" in path.read_text(encoding="utf-8")
