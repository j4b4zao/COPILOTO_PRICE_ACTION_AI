...
def test_real_observed_no_data_reinitialization_sequence():
    initializing_cycles = [
        (3545, "2026-09-16T12:42:14.342"),
        (3547, "2026-09-16T12:42:16.151"),
        (3549, "2026-09-16T12:42:17.988"),
        (3550, "2026-09-16T12:42:18.958"),
        (3551, "2026-09-16T12:42:20.112"),
        (3552, "2026-09-16T12:42:21.047"),
        (3553, "2026-09-16T12:42:22.009"),
        (3554, "2026-09-16T12:42:22.975"),
        (3556, "2026-09-16T12:42:24.732"),
        (3558, "2026-09-16T12:42:26.538"),
        (3560, "2026-09-16T12:42:28.322"),
        (3562, "2026-09-16T12:42:30.204"),
        (3563, "2026-09-16T12:42:31.214"),
    ]
    ...
    samples = [
        sample(
            3541,
            "VALID",
            "2026-09-16T12:42:10.517",
            delta=174.0,
        ),
        sample(
            3543,
            "NO_DATA",
            "2026-09-16T12:42:12.456",
        ),
    ]
    ...
    samples.extend(
        ...
        for cycle, timestamp in initializing_cycles
    )
    ...
    assert episode["classification"] == "TRANSIENT_RECOVERED"
    assert episode["previous_ready"]["cycle"] == 3541
    assert episode["failure"]["cycle"] == 3543
    assert episode["failure_status"] == "NO_DATA"
    assert episode["initializing_sample_count"] == 13
    assert episode["additional_failure_sample_count"] == 0
    assert episode["recovery"]["cycle"] == 3564
    assert episode["recovery"]["delta_status"] == "VALID"
    assert episode["failure_to_recovery_seconds"] == 19.765
    assert episode["previous_ready_to_recovery_seconds"] == 21.704

    assert episode["status_sequence"] == (
        ["NO_DATA"]
        + ["INITIALIZING"] * 13
        + ["VALID"]
    )
...
def test_unknown_delta_status_is_not_silently_treated_as_known_failure():
    report = audit_session(
        payload(
            [
                sample(
    ...
    assert report["failure_sample_count"] == 0
    assert report["episode_count"] == 0
    assert report["unknown_status_sample_count"] == 1
