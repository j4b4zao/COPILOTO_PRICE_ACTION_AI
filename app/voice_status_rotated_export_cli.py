"""
BookDiagnostics RC69 - Voice Status Rotated Export CLI.

CLI explicito para gerar snapshots JSON rotacionados via RC68.
Nao possui capacidade de audio e nao se conecta ao loop principal.
"""

from __future__ import annotations

import argparse

from analysis.replay.book_diagnostics_voice_config import VoiceConfig
from core.system_initializer import SystemInitializer


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Exportacao rotacionada do status de voz do COPILOTO PRICE ACTION AI"
    )
    parser.add_argument("directory", help="Diretorio de destino dos snapshots JSON")
    parser.add_argument("--keep", type=int, default=20, help="Quantidade maxima de snapshots a manter")
    parser.add_argument("--prefix", default="voice_status", help="Prefixo seguro dos arquivos")
    parser.add_argument("--enabled", action="store_true", help="Avalia o servico como habilitado sem iniciar fala")
    parser.add_argument("--backend", default="NULL_TTS")
    parser.add_argument("--language", default="pt-BR")
    parser.add_argument("--profile", default="BRITISH_CALM_PRECISE_ASSISTANT")
    parser.add_argument("--rate", type=float, default=1.0)
    return parser


def run(argv: list[str] | None = None) -> int:
    args = build_parser().parse_args(argv)
    config = VoiceConfig(
        enabled=bool(args.enabled),
        backend=str(args.backend).strip().upper(),
        language=str(args.language).strip(),
        voice_profile=str(args.profile).strip(),
        speech_rate=float(args.rate),
    )
    system = SystemInitializer(voice_config=config).inicializar()
    result = system.voice.export_status_rotated(
        args.directory,
        keep=int(args.keep),
        prefix=str(args.prefix).strip(),
    )

    print("=== VOICE STATUS ROTATED EXPORT ===")
    print(f"PATH: {result.path}")
    print(f"STATUS: {result.status}")
    print(f"KEPT: {result.kept}")
    print(f"REMOVED: {len(result.removed_paths)}")
    print(f"PREFIX: {result.prefix}")
    print(f"READONLY: {result.readonly}")

    return 0


def main() -> None:
    raise SystemExit(run())


if __name__ == "__main__":
    main()
