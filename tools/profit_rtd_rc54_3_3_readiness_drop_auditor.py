...
def audit(path):
    ...
    for idx, sample in enumerate(samples):
        ready = bool(sample.get('context_ready'))
        ...
        if current_ready is None:
            ...
            target.append({'start_cycle': run_start, 'end_cycle': previous_cycle})
            current_ready = ready
            run_start = cycle
    ...
    longest_drop_run = max((r['end_cycle'] - r['start_cycle'] + 1 for r in drop_runs), default=0)
    longest_ready_run = max((r['end_cycle'] - r['start_cycle'] + 1 for r in ready_runs), default=0)
