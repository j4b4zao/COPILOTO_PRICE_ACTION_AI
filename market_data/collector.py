...
class Collector:
    ...
    def __init__(self, excel=None, reader=None, multi_timeframe=None, renko_state=None,
        ...
        self.chart_mode = ChartMode.normalize(chart_mode if chart_mode is not None else CHART_MODE)
        brick_size = renko_brick_size if renko_brick_size is not None else RENKO_BRICK_SIZE
        self.multi_timeframe = multi_timeframe if multi_timeframe is not None else MultiTimeframeState()
        self.renko = renko_state if renko_state is not None else RenkoState(brick_size=brick_size)
        self.order_flow = order_flow_state if order_flow_state is not None else OrderFlowState()
    ...
    def get_data(self):
        ...
        if not math.isfinite(volume) or volume < 0:
            print(f"[DATA WARNING] Volume inválido recebido: {volume!r}. Volume será tratado como 0.0.")
            volume = 0.0
        ...
        if integrity.duplicate:
            ...
            self._observe_delta_source(integrity, aggression_buy, aggression_sell)
            print("[DATA WAIT] Snapshot do Profit sem mudança; ciclo ignorado.")
            return None
        timestamp = self.clock.now()
        self.last_new_candles = self.multi_timeframe.update_tick(
            symbol=ativo, price=close, cumulative_volume=volume, timestamp=timestamp)
        if self.chart_mode == ChartMode.RENKO:
            self.last_closed_renko_bricks = self.renko.update_tick(
                symbol=ativo, price=close, cumulative_volume=volume, timestamp=timestamp)
        self._update_order_flow(aggression_buy=aggression_buy, aggression_sell=aggression_sell, price=close)
        self._observe_delta_source(integrity, aggression_buy, aggression_sell)
        ...
        return AnalysisContext(market=self.market, multi_timeframe=self.multi_timeframe,
                               order_flow_state=self.order_flow)

    def _observe_delta_source(self, integrity, aggression_buy, aggression_sell):
        self.source_diagnostics.observe(integrity=integrity, aggression_buy=aggression_buy,
                                        aggression_sell=aggression_sell, order_flow=self.order_flow)
    ...
    def _update_order_flow(self, *, aggression_buy, aggression_sell, price):
        if aggression_buy is None or aggression_sell is None:
            self.order_flow.mark_unavailable(); return
        if self.chart_mode == ChartMode.NORMAL:
            self.order_flow.update(cumulative_buy=aggression_buy, cumulative_sell=aggression_sell,
                                   price=price, sampling_mode="TICK", source_units=1); return
        if self.order_flow.cumulative_buy is None:
            self.order_flow.update(cumulative_buy=aggression_buy, cumulative_sell=aggression_sell,
                                   price=price, sampling_mode="RENKO_CLOSE", source_units=0); return
        if self.last_closed_renko_bricks <= 0:
            self.order_flow.mark_waiting("RENKO_CLOSE"); return
        self.order_flow.update(cumulative_buy=aggression_buy, cumulative_sell=aggression_sell,
                               price=price, sampling_mode="RENKO_CLOSE",
    ...
    def to_optional_float(value):
        ...
        if isinstance(value, (int, float)): converted = float(value)
        else:
            try:
                text = str(value).strip()
        ...
        if not math.isfinite(converted) or converted < 0: return None
        return converted
